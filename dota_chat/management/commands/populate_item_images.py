...
class Command(BaseCommand):
    help = 'Populates item images from OpenDota urls'
    ...
    def handle(self, *args, **options):
            ...
            for item in items:
                ...
                try:
                    ...
                    imgStaticFile = os.path.join(settings.STATIC_ROOT,os.path.basename(item.opendota_img.split('?')[0]))

                    imgQueryStr = '{}{}'.format(imgURLBaseStr,item.opendota_img)
                    ...
                    item.img.save(
                        os.path.basename(item.opendota_img.split('?')[0]),
